...
local HomeSection = HomeTab:Section({
 ...
 HomeTab:Paragraph({
    Title = "About",
    Desc = "About Axiora Is We. Make Scripts so Making. Op Scripts You can find the Scripts in my Discord Community ",
    Color = "Grey",
})

HomeTab:Paragraph({
    Title = "Changelogs. 0.2",
    Desc = "Sewh Recode By Viper\nAdded AntiRagdoll disable Ragdoll and falldamge",
    Color = "Grey",
})

HomeTab:Paragraph({
    Title = "Axiora Supported Games",
    Desc = "VolleyBall Legend\nSomething evil. Will happen\nfrom now thats Two. Game Axiora made We will continuous Making Scripts And Loving yall\nFree to Suggest in my Discord Server! ",
    Color = "Grey",
})
...
local SpeedInput = PlayerTab:Input({
    ...
    Callback = function(input) 
        ...
        if speed and speed >= 1 and speed <= 1000 then -- Reasonable limits
            ...
            WindUI:Notify({
                Title = "Invalid Input",
                Content = "Please enter a number between 1 and 1000!",
                Duration = 4,
                Icon = "alert-triangle",
            })
        end
    ...
    Callback = function(state) 
        ...
        if state then
            ...
            WindUI:Notify({
                ...
                Content = "Custom walkspeed enabled: " .. getgenv().CustomWalkSpeed, -- FIXED variable name
                Duration = 3,
                Icon = "zap",
            })
        else
    ...
    Callback = function(input) 
        ...
        if jumpPower and jumpPower >= 1 and jumpPower <= 1000 then -- Reasonable limits
            ...
            WindUI:Notify({
                ...
                Content = "Custom jump power set to: " .. jumpPower,
                Duration = 3,
                Icon = "zap",
            })
...
local ApplyJumpToggle = PlayerTab:Toggle({
    ...
    Callback = function(state) 
        ...
        if state then
            ...
            WindUI:Notify({
                Title = "Jump Enabled!",
                Content = "Custom jump power enabled: " .. getgenv().CustomJumpPower,
                Duration = 3,
                Icon = "zap",
            })
        else
        ...
        pcall(function()
            ...
            if state then
                WindUI:Notify({
                    ...
                    Content = "You can now sprint infinitely!",
                    Duration = 3,
                    Icon = "infinity",
                })
            else
                WindUI:Notify({
                    ...
                    Content = "Sprint stamina reset to normal",
                    Duration = 3,
                    Icon = "infinity",
                })
            end
...
local GodModeToggle = PlayerTab:Toggle({
    Title = "Semi GodMode",
    Desc = "does only Godmode in Fall Damage",
...
WindUI:Notify({
    Title = "Error",
    Content = "Failed to Tp Report on my Discord Community if you find bug",
    Duration = 3, -- 3 seconds
    Icon = "x",
})
end
